...
def _run_chat_app(llm_service_mock) -> AppTest:
...
class TestChatUI:
    ...
    def test_chat_ui(self, llm_service_mock):
        at = _run_chat_app(llm_service_mock)
    ...
    def test_chat_ui_with_toggle_on_and_messages_should_show_clear_btn(self, llm_service_mock):
        at = _run_chat_app(llm_service_mock)
        ...
        at.chat_input(key="chat_input_key").set_value("Additional Input").run()

        assert len(at.session_state.messages) == 2
        assert at.session_state.messages[1]['role'] == "user"
        assert at.session_state.messages[1]['content'] == "Additional Input"
