        ...
        if submitted and user_query:
            with st.spinner("Analyzing database db and generating query..."):
                if not contains_forbidden_words(user_query):
                    ...
                    if st.session_state.generated_sql and response != st.session_state.generated_sql:
                        st.session_state.show_df = False
        ...
        if st.session_state.generated_sql:
            ...
            if st.button("Execute Query"):
                ...
                with st.spinner("Executing query..."):
                    st.session_state.query_result = self.db_service.execute_query(st.session_state.generated_sql)

            # if the query is different, return set show_df to false
            if st.session_state.show_df and st.session_state.query_result is not None:
                st.subheader("Data Overview")
